...
Formally, let $p_Z(z)$ be a random variable with a simple base distribution (typically a standard Gaussian is used), and let $f: \mathbb{R}^D \to \mathbb{R}^D$ be an invertible transformation. The density of $y = f(z)$ is given by the change of variables formula:
\begin{equation}
...
RealNVP \citep{dinh2016density} introduces affine coupling layers that partition input dimensions and apply a tractable affine transformation. Given a $D$-dimensional input $z$, partition it into two parts: $z_{1:d}$ (first $d$ dimensions) and $z_{d+1:D}$ (remaining $D-d$ dimensions). The forward transformation is:
\begin{align}
y_{1:d} &= z_{1:d} \\
y_{d+1:D} &= z_{d+1:D} \odot \exp(s(z_{1:d})) + t(z_{1:d})
\end{align}
where $s: \mathbb{R}^d \to \mathbb{R}^{D-d}$ and $t: \mathbb{R}^d \to \mathbb{R}^{D-d}$ are the scale and translation functions, typically parameterized by neural networks.
...
A single coupling layer leaves $d$ dimensions unchanged. To ensure all dimensions are transformed, RealNVP stacks multiple coupling layers with alternating binary masks. For example, with mask patterns $[1,0,1,0,\ldots]$ and $[0,1,0,1,\ldots]$ alternating across layers, every dimension is eventually subject to a learned transformation.
...
\subsection{TabPFN (Tabular Prior-data Fitted Network)}
\label{sec:tabpfn}

\subsubsection{Mathematical Foundation}

TabPFN \citep{hollmann2023tabpfn} represents a fundamentally different approach to tabular prediction. Rather than learning model parameters from the training data through gradient-based optimization, TabPFN is a pre-trained transformer that performs in-context learning. Given a training dataset as input context, it directly outputs predictions for new query points without any task-specific parameter updates.
...
\begin{equation}
p(y \mid X_{\text{test}}, X_{\text{train}}, y_{\text{train}}) = \text{TabPFN}(X_{\text{test}}; X_{\text{train}}, y_{\text{train}})
\end{equation}
...
\subsubsection{The TabPFN Prior: Structural Causal Models}
...
\begin{itemize}
\item \textbf{Linear transformations}: $f_k(\mathbf{z}) = \mathbf{W}\mathbf{z} + \mathbf{b}$
\item \textbf{Small neural networks}: Multi-layer perceptrons with randomly sampled architectures, activation functions, and weight initializations
\item \textbf{Decision tree splits}: Threshold-based partitioning functions
\item \textbf{Discretization operations}: Binning continuous values into categories
\end{itemize}
...
\textbf{Step 3: Feature and Target Extraction.} Once the flat prior of diverse data-generating processes has been encoded, feature positions $F$ and target positions $T$ are randomly sampled from among the graph nodes. The intermediate data representations at these sampled positions become the features $X$ and targets $y$ of the synthetic datasets.

\textbf{Step 4: Post-processing.} The extracted data undergoes post-processing to enhance realism and complexity, including normalization, noise injection, and handling of missing values.
...
\subsubsection{Two-Way Attention Architecture}
...
\textbf{Row Attention (Inter-Sample).} Each cell attends to the same feature across all other samples (its column). For feature $j$, the attention operates over the sequence $\{z_{1,j}, z_{2,j}, \ldots, z_{N+1,j}\}$:
\begin{equation}
\text{RowAttn}(Z)_{i,j} = \text{Attention}(z_{i,j}, \{z_{k,j}\}_{k=1}^{N+1})
\end{equation}
...
\textbf{Column Attention (Intra-Sample).} Each cell attends to the other features within its row (its sample). For sample $i$, the attention operates over the sequence $\{z_{i,1}, z_{i,2}, \ldots, z_{i,d+1}\}$:
\begin{equation}
\text{ColAttn}(Z)_{i,j} = \text{Attention}(z_{i,j}, \{z_{i,l}\}_{l=1}^{d+1})
\end{equation}
...
For imputation tasks requiring stochastic samples rather than point predictions, TabPFN provides access to the full predictive distribution. The model outputs a parameterized distribution from which arbitrary quantiles can be extracted. For a given quantile $\tau$, the prediction is:
\begin{equation}
y_\tau = F^{-1}_{\text{pred}}(\tau)
\end{equation}
...
Such implementation has demonstrated that QRF is well-suited for wealth imputation as it naturally captures the entire conditional distribution between predictors and wealth, preserving heteroskedasticity and skewness while being robust to outliers, which are prevalent in wealth data \citet{juaristi2025microimpute}.
